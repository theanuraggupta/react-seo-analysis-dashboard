...
const SEODashboard = () => {
    const load = (args) => {
        ...
        args.accumulation.theme = (selectedTheme.charAt(0).toUpperCase() + selectedTheme.slice(1)).replace(/contrast/i, 'Contrast').replace(/-dark/i, "Dark");
    };
    const Chartload = (args) => {
        let selectedTheme = location.hash.split('/')[1];
        selectedTheme = selectedTheme ? selectedTheme : 'Material';
    ...
    const Mapload = (args) => {
        let selectedTheme = location.hash.split('/')[1];
        selectedTheme = selectedTheme ? selectedTheme : 'Material';
        args.maps.theme = (selectedTheme.charAt(0).toUpperCase() + selectedTheme.slice(1)).replace(/contrast/i, 'Contrast').replace(/-dark/i, "Dark");
    };
    ...
    const map = () => {
        ...
        return (<div style={{ height: "100%", width: "100%" }}>
                <MapsComponent id="maps" load={Mapload.bind(this)} zoomSettings={{ enable: false }} legendSettings={{ visible: false }}>
                        ...
                        <LayersDirective>
                            <LayerDirective shapeData={new MapAjax('https://ej2.syncfusion.com/react/demos/src/dashboard-layout/world-map.json')} shapePropertyPath='continent' shapeDataPath='continent' dataSource={datasource.default} shapeSettings={{ colorValuePath: 'color' }}>
                                <MarkersDirective>
                                    ...
                                    <MarkerDirective visible={true} shape='Image' imageUrl='https://ej2.syncfusion.com/react/demos/src/dashboard-layout/ballon.png' height={20} width={20} offset={{ y: -10, x: 0 }} animationDuration={0} tooltipSettings={{ visible: true, valuePath: 'name' }} dataSource={mapData}></MarkerDirective>
                                </MarkersDirective>
                            </LayerDirective>
                        </LayersDirective>
                    </MapsComponent>
                            ...
                            <div className="dashboardParent">
                                <DashboardLayoutComponent id="analytic_dashboard" cellAspectRatio={100 / 85} cellSpacing={cellSpacing} columns={6}>
                                    <PanelsDirective>
                                        ...
                                        <PanelDirective sizeX={2} sizeY={1} row={0} col={2} content={card2}></PanelDirective>
                                        <PanelDirective sizeX={2} sizeY={1} row={0} col={4} content={card3}></PanelDirective>
                                        <PanelDirective sizeX={2} sizeY={2} row={1} col={0} content={pie.bind(this)} header="<div>Active Visitors</div>"></PanelDirective>
                                        <PanelDirective sizeX={2} sizeY={2} row={1} col={2} content={map.bind(this)} header="<div>Regional Map</div>"></PanelDirective>
                                        <PanelDirective sizeX={2} sizeY={2} row={1} col={4} content={colChart.bind(this)} header="<div>Visitors by Type</div>"></PanelDirective>
                                        <PanelDirective sizeX={2} sizeY={2} row={3} col={0} content={pieChart.bind(this)} header="<div>Useage Statistics</div>"></PanelDirective>
                                        <PanelDirective sizeX={4} sizeY={2} row={3} col={2} content={visitorsChart.bind(this)} header="<div>Traffic History</div>"></PanelDirective>
                                    </PanelsDirective>
